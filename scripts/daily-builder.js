...
5. Zustand Store API: The store 'useToolStore' exported from '@/store/useToolStore' has these exact properties and methods:
   ...
   * addToHistory(slug: string): void (Do NOT use non-existent names like "addToolToHistory"!)
   * clearHistory(): void
6. No Shadcn/UI or Sonner: Do NOT import components from '@/components/ui/*' or use 'sonner' (which are not installed). Instead, build clean raw HTML inputs, buttons, selects, and textareas styled with Tailwind CSS classes, and use 'react-hot-toast' for notifications.
7. Navigation and Sidebar: If you add a new tool page, you MUST update 'src/lib/tools.tsx' (which holds the tool list array).
